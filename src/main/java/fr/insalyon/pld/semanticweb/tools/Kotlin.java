...
public class Kotlin {
  ...
  public static <R> List<R> mutableListOf(Collection<R> collection) {
  ...
  public static <K, V> Map<K, V> mutableMapOf(Set<Map.Entry<K, V>> pairList) {
    Map<K, V> result = new LinkedHashMap<>();
    pairList.forEach(pair -> result.put(pair.getKey(), pair.getValue()));
    return result;
  }
